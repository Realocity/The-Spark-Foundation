...
<body>
    ...
    <div class="container-fluid">
        ...
        if (isset($_REQUEST['c_id'])) {
            $sid = $_GET['c_id'];
            $sql = "SELECT * FROM  customer where c_id=$sid";
        ...
        <form method="post" name="tcredit" class="tabletext"><br>
            ...
            <div class="text-center">
                <button class="btn mt-3 waves-effect waves-light amber accent-4" name="submit" type="submit" id="myBtn">Send<i class="material-icons right">attach_money</i></button>
            </div>
            <br>
        </form>
    </div>
...
if (isset($_POST['submit'])) {
    ...
    // constraint to check zero values
        ...
        $sql = "SELECT * from customer where c_id=$to";
        $query = mysqli_query($conn, $sql);
        ...
        $sql = "UPDATE customer set c_balance=$newbalance where c_id=$from";
        mysqli_query($conn, $sql);

        // adding amount to reciever's account
        $newbalance = $sql2['c_balance'] + $amount;
        $sql = "UPDATE customer set c_balance=$newbalance where c_id=$to";
        mysqli_query($conn, $sql);
